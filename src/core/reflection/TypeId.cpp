...
namespace Core {
	...
	std::unique_ptr<TypeInfo> createBasicTypeInfo(size_t size, std::function<void*()> createFunc,
		...
		typeInfo->getTypeTraits() = BasicTypeTraits {
			TypeId(*typeInfo),
			std::move(createFunc),
	...
	std::unique_ptr<TypeInfo> createOptionalTypeInfo(size_t size, TypeId valueType,
		...
		auto typeInfo = std::make_unique<TypeInfoImpl>(size);
		typeInfo->getTypeTraits() = OptionalTypeTraits {
			std::move(valueType),
			std::move(applyFunc),
			std::move(getFunc)
		};
	...
	std::unique_ptr<TypeInfo> createVectorTypeInfo(size_t size, TypeId valueType,
		...
		auto typeInfo = std::make_unique<TypeInfoImpl>(size);
		typeInfo->getTypeTraits() = VectorTypeTraits {
			std::move(valueType),
			std::move(applyFunc),
	...
	std::unique_ptr<TypeInfo> createMapTypeInfo(size_t size, TypeId keyType, TypeId valueType,
		...
		auto typeInfo = std::make_unique<TypeInfoImpl>(size);
		typeInfo->getTypeTraits() = MapTypeTraits{
			std::move(keyType),
			std::move(valueType),
			std::move(applyFunc),
			std::move(forEachFunc)
	...
	bool TypeId::operator==(const TypeInfo& other) const {
		return mTypeInfo == &other;
	}
